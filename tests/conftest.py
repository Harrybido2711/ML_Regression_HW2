...
def grade_assignment(request):
        ...
        if name in rubric:
            if rubric[name]['weight'] == 'required':
                if not result:
                    ...
                    report['notes'][name] = "PASS:"
                    report['tests_passed'] += 1
                    total_tests += 1
            else:
                dependencies_satisifed = "yes"
                for dependency in rubric[name].get('depends', []):
                    if dependency in test_result:
                        if not test_result[dependency]:
                            dependencies_satisifed = "no"
                    elif dependencies_satisifed == "yes":
                        dependencies_satisifed = "maybe"
                ...
                if not result:
                    ...
                    depends_str = ", ".join(rubric[name]["depends"])
                    report['notes'][name] = (
                        f'PASS (but MISSING {depends_str}):'
                    )
                    report['tests_passed'] += 1
                else:
    ...
    # for i, note in report['notes'].items():
        note = report['notes'].get(key, "SKIP")
        print(f'{note} {key}')
    print("======================")
