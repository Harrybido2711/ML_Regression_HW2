...
class Perceptron():
    ...
    def fit(self, X, y):
        ...
        Args:
            X (np.ndarray): a NxD array containing N examples each with D features.
        ...
        self.weights = np.zeros(X2.shape[1])
    ...
    def update_weights(self, X, y):
        """
        Perform one iteration of updates for the Perceptron algorithm Note:
        don't forget to use `self.learning_rate` to scale the amount of the
        update.
        ...
        Args:
            X: the Nx(D+1) matrix of features, including an intercept
    ...
    def predict(self, X):
        ...
        Note: Keep the `add_intercept` call to ensure you include the intercept
        Note: you will have to use `self.map_pm1_to_01` to convert your
            predictions to {0, 1} so they match the labels.

        Args:
            X (np.ndarray): 2D array of shape (N, D) containing real-valued
                inputs.
        Returns:
            predictions (np.ndarray): Predictions array of shape (N, 1) with
                the outputs converted to {0, 1} labels.
        """
