...
def generate_random_numbers(degree, N, amount_of_noise):
    ...
    Returns:     
        ...
        Use src.random to generate `coefficient value coefs`: an array of shape
            ...
            between -10 and 10.
        Use src.random to generate `noise variable n`: an array of shape (N, 1)
            that contains floats chosen in the normal distribution. The mean is
            0 and the standard deviation is `amount_of_noise`.
...
def generate_regression_data(degree, N, amount_of_noise=1.0):
    ...
    Do not use these numpy or internal functions: polynomial, polyfit, polyval, __dict__, lstsq, getattr, globals

    Args:
        degree (int): degree of the Polynomial that maps from the original x to y
